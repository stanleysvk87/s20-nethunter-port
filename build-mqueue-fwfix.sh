...
output_dir="${port_dir}/out-mqueue-fwfix"
...
docker run --rm \
    ...
    bash -euc '
        ...
        config=/work/out-mqueue-fwfix/.config

        gzip -cd /work/running.config.gz > "${config}"
        scripts/config --file "${config}" --set-str LOCALVERSION "-nh-z3s-rt2800-mqfw"
        ...
        make O=/work/out-mqueue-fwfix "${toolchain_args[@]}" olddefconfig
        ...
        mkdir -p /work/out-mqueue-fwfix/firmware/npu
        install -m 0644 /firmware-input/npu/NPU.bin \
            /work/out-mqueue-fwfix/firmware/npu/NPU.bin
        install -m 0644 /firmware-input/valhall-1691526.wa \
            /work/out-mqueue-fwfix/firmware/valhall-1691526.wa
        install -m 0644 /firmware-input/rt2870.bin \
            /work/out-mqueue-fwfix/firmware/rt2870.bin
        ...
        make O=/work/out-mqueue-fwfix "${toolchain_args[@]}" -j"${KERNEL_JOBS}"
    '
