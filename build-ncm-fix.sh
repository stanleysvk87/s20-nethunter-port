...
output_dir="${port_dir}/out-ncm-fix"
...
docker run --rm \
    ...
    bash -euc '
        ...
        config=/work/out-ncm-fix/.config

        gzip -cd /work/running.config.gz > "${config}"
        scripts/config --file "${config}" --set-str LOCALVERSION "-nh-z3s-ncmfix"
        ...
        make O=/work/out-ncm-fix "${toolchain_args[@]}" olddefconfig
        ...
        mkdir -p /work/out-ncm-fix/firmware/npu
        install -m 0644 /firmware-input/npu/NPU.bin \
            /work/out-ncm-fix/firmware/npu/NPU.bin
        install -m 0644 /firmware-input/valhall-1691526.wa \
            /work/out-ncm-fix/firmware/valhall-1691526.wa
        install -m 0644 /firmware-input/rt2870.bin \
            /work/out-ncm-fix/firmware/rt2870.bin
        ...
        make O=/work/out-ncm-fix "${toolchain_args[@]}" -j"${KERNEL_JOBS}"
    '
