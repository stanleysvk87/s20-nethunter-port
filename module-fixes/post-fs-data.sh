...
# Keep the target relative so app UIDs do not have to traverse /data/adb.
ln -sf "$busybox_nh" busybox_nh

# LineageOS exposes /system/xbin as 0750 root:shell. Publish the selected
# binary through /system/bin as well so the NetHunter app can detect it.
if [ "$XBIN" != "$BIN" ]; then
  ln -f "$XBIN/$busybox_nh" "$BIN/$busybox_nh"
  ln -sf "$busybox_nh" "$BIN/busybox_nh"
fi
