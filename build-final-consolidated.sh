...
output_dir="${port_dir}/out-final-consolidated"
...
docker run --rm \
    ...
    bash -euc '
        ...
        config=/work/out-final-consolidated/.config

        gzip -cd /work/running.config.gz > "${config}"
        scripts/config --file "${config}" --set-str LOCALVERSION "-nh-z3s-final"
        ...
        make O=/work/out-final-consolidated "${toolchain_args[@]}" olddefconfig
        ...
        mkdir -p /work/out-final-consolidated/firmware/npu
        install -m 0644 /firmware-input/npu/NPU.bin \
            /work/out-final-consolidated/firmware/npu/NPU.bin
        install -m 0644 /firmware-input/valhall-1691526.wa \
            /work/out-final-consolidated/firmware/valhall-1691526.wa
        install -m 0644 /firmware-input/rt2870.bin \
            /work/out-final-consolidated/firmware/rt2870.bin
        ...
        make O=/work/out-final-consolidated "${toolchain_args[@]}" -j"${KERNEL_JOBS}"
    '
