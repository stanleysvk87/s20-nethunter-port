...
output_dir="${port_dir}/out-bt-eventfilter-fix"
...
docker run --rm \
    ...
    bash -euc '
        ...
        config=/work/out-bt-eventfilter-fix/.config

        gzip -cd /work/running.config.gz > "${config}"
        scripts/config --file "${config}" --set-str LOCALVERSION "-nh-z3s-btfix2"
        ...
        make O=/work/out-bt-eventfilter-fix "${toolchain_args[@]}" olddefconfig
        ...
        mkdir -p /work/out-bt-eventfilter-fix/firmware/npu
        install -m 0644 /firmware-input/npu/NPU.bin \
            /work/out-bt-eventfilter-fix/firmware/npu/NPU.bin
        install -m 0644 /firmware-input/valhall-1691526.wa \
            /work/out-bt-eventfilter-fix/firmware/valhall-1691526.wa
        install -m 0644 /firmware-input/rt2870.bin \
            /work/out-bt-eventfilter-fix/firmware/rt2870.bin
        ...
        make O=/work/out-bt-eventfilter-fix "${toolchain_args[@]}" -j"${KERNEL_JOBS}"
    '
