...
output_dir="${port_dir}/out-bt-hcisock-fix"
...
docker run --rm \
    ...
    bash -euc '
        ...
        config=/work/out-bt-hcisock-fix/.config

        gzip -cd /work/running.config.gz > "${config}"
        scripts/config --file "${config}" --set-str LOCALVERSION "-nh-z3s-btfix"
        ...
        make O=/work/out-bt-hcisock-fix "${toolchain_args[@]}" olddefconfig
        ...
        mkdir -p /work/out-bt-hcisock-fix/firmware/npu
        install -m 0644 /firmware-input/npu/NPU.bin \
            /work/out-bt-hcisock-fix/firmware/npu/NPU.bin
        install -m 0644 /firmware-input/valhall-1691526.wa \
            /work/out-bt-hcisock-fix/firmware/valhall-1691526.wa
        install -m 0644 /firmware-input/rt2870.bin \
            /work/out-bt-hcisock-fix/firmware/rt2870.bin
        ...
        make O=/work/out-bt-hcisock-fix "${toolchain_args[@]}" -j"${KERNEL_JOBS}"
    '
